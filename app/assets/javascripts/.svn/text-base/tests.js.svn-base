...
$(function(){
  ...
  var timeID = setInterval(function(){
    ...
    if (time_count == 0) {
      $('.hide-box div').text('時間切れ').addClass('label-warning');
      ...
      $.ajax({
        ...
        success: function(data) {
          if (question_count == 0) {
            ...
            $('.user_id').val(user_id);
          } else if (question_count < 11 && question_count % 5 != 0) {
            ...
            $('.user_id').val(user_id);
            $(".result_array_user").val(data["result_array_user"])
            Register_user_score(user_id, category_id, data["rd_user"], return_score)
          }
        }
      });
...
function Register_user_score(user_id,category_id, rd_user, score){
  query = {};
  query['user_id'] = user_id;
  ...
  $.ajax({
    ...
    data: query,
    success: function(data){
      $('.hide-box div').text('テスト終了').addClass('label-info');
      $('.hide-box').css({display: 'block'});
      $('.result-box').css({display: 'block'});
      $('#next_question').val("終了");
      $('#next_question').addClass('btn-inverse');
      $('#next_question').attr('id','finish_btn');
      $('.question_point').text("終了");
      $('.test_sum_score').text(score_s);
      }
    });
}
